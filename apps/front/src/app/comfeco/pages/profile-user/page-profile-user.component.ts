...
export class PageProfileUserComponent implements OnInit, OnDestroy {
  ...
  newInsigniasUserSubscription$:Subscription;
  insigniasUserSubscription$:Subscription;
  allInsigniasSubscription$:Subscription;

  constructor(
    private _serviceProfile: PageTabsProfileService,
    private _serviceInsignias: InsigniasService,
  ...
  ngOnDestroy() {
    ...
    this.newInsigniasUserSubscription$?.unsubscribe();
    this.insigniasUserSubscription$?.unsubscribe();
    this.allInsigniasSubscription$?.unsubscribe();
  }
  ...
  subscriptionInsignias() {
    this.spinner.show();
    this.newInsigniasUserSubscription$ = this._serviceProfile.userInsignias$.subscribe(insigniasChanged => {
      if(!!this.insignias) {
  ...
  subscriptionEvents() {
    this.addedEventsSubscription$ = this._serviceProfile.addedEvent$.subscribe(eventsChanged => {
      ...
      this.events = [ eventsChanged, ...this.events ];
      this.completeRecentActivity();
      this.spinner.hidde();
    });
  ...
  completeAllInsignias() {
    this.spinner.show();
    this.allInsigniasSubscription$ = this._serviceInsignias.insignias()
      .subscribe(
        (resp:any) => {
          if(resp.success) {
  ...
  completeRecentActivity() {
    this.spinner.show();
    this._profileService.recentActivity()
      .subscribe(
        (resp:any) => {
  ...
  processingInsignias(insignias:InsigniaDto[]) {
    if(!!insignias) {
      ...
      this.insigniasUserSubscription$ = this._serviceInsignias.userInsignias()
        .subscribe(
          (resp:any) => {
            if(resp.success) {
